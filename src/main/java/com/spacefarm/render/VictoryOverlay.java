...
public class VictoryOverlay {
    ...
    public void render(int sw, int sh) {
        ...
        batch.begin();
        titleFont.setColor(0.15f, 0.90f, 0.40f, 1f);
        layout.setText(titleFont, "ПЕРЕМОГА!");
        float scaleX = Math.min(1f, (panelW * 0.82f) / layout.width);
        titleFont.getData().setScale(scaleX, 1f);
        layout.setText(titleFont, "ПЕРЕМОГА!");
        titleFont.draw(batch, "ПЕРЕМОГА!", cx - layout.width * 0.5f, py + panelH - panelH * 0.08f);
        ...
        batch.begin();
        subFont.setColor(0.70f, 1.00f, 0.55f, 1f);
        layout.setText(subFont, "Дерево розцвіло!");
        subFont.draw(batch, "Дерево розцвіло!", cx - layout.width * 0.5f, py + panelH - panelH * 0.33f);

        bodyFont.setColor(0.30f, 0.60f, 0.30f, 1f);
        String flavour = "Атмосфера відновлена. Планета знову живе.";
        ...
        drawButton(btnRestart,  "ЗІГРАТИ ЩЕ РАЗ", 0.05f, 0.22f, 0.08f, 0.20f, 0.80f, 0.30f);
        drawButton(btnMainMenu, "ГОЛОВНЕ МЕНЮ",    0.06f, 0.12f, 0.10f, 0.20f, 0.60f, 0.55f);

        batch.begin();
        hintFont.setColor(0.20f, 0.45f, 0.25f, 1f);
        String hint = "Ти врятував цей світ. Дякуємо, фермере!";
    ...
    private void drawPanel(float px, float py, float pw, float ph) {
        ...
        shapeRenderer.setColor(0.02f, 0.10f, 0.04f, 0.97f);
        shapeRenderer.rect(px, py, pw, ph);
        shapeRenderer.setColor(0.05f, 0.55f, 0.15f, 1f);
        shapeRenderer.rect(px, py + ph - border, pw, border);
        shapeRenderer.rect(px, py,               pw, border);
        shapeRenderer.setColor(0.04f, 0.40f, 0.10f, 1f);
        ...
        shapeRenderer.setColor(0.10f, 0.75f, 0.25f, 0.90f);
        shapeRenderer.rect(px, py, pw, ph);
        shapeRenderer.setColor(0.06f, 0.40f, 0.12f, 0.60f);
    ...
    private void drawHRule(float cx, float y, float w) {
        shapeRenderer.begin(ShapeRenderer.ShapeType.Filled);
        shapeRenderer.setColor(0.08f, 0.60f, 0.18f, 1f);
